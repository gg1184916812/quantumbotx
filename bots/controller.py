...
def perbarui_bot(bot_id: int, data: dict):
    ...
    # 确保 custom_params 是 dict，如果不是则转为 {}
    if not isinstance(custom_params, dict):
        custom_params = {}
    data['strategy_params'] = json.dumps(custom_params)  # <--- 关键：转为 JSON 字符串

    # Filter data untuk hanya argumen yang diharapkan
    expected_args = [
...
def get_bot_analysis_data(bot_id: int):
    """Mengambil data analisis terakhir dari instance bot, termasuk AI prediction."""
    bot = active_bots.get(bot_id)
    if bot and hasattr(bot, 'last_analysis'):
        analysis = bot.last_analysis.copy() if bot.last_analysis else {}
        # Tambahkan AI prediction jika ada
        if hasattr(bot, 'last_ml_result') and bot.last_ml_result:
            analysis['ai_state'] = bot.last_ml_result.get('state', 0)
            analysis['ai_confidence'] = bot.last_ml_result.get('confidence', 0.0)
            analysis['ai_probabilities'] = bot.last_ml_result.get('probabilities', [])
        if hasattr(bot, 'current_ai_state'):
            analysis['current_ai_state'] = bot.current_ai_state
        if hasattr(bot, 'strategy_name'):
            analysis['strategy_name'] = bot.strategy_name
        return analysis

    # For inactive bots, generate analysis on-demand (existing code remains)
    bot_data = queries.get_bot_by_id(bot_id)
    if not bot_data:
        return None
    # ... 其余原有代码保持不变 ...
