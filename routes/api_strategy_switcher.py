...
# ============================================================
# ML Prediction Status for all bots
# ============================================================

@api_strategy_switcher.route('/ml-status', methods=['GET'])
def get_ml_status():
    """获取所有活跃 Bot 的 ML 预测状态"""
    try:
        ...
        for bot_id, bot in active_bots.items():
            if hasattr(bot, 'last_ml_result') and bot.last_ml_result:
                result[str(bot_id)] = {
                    'state': bot.last_ml_result.get('state', 0),
                    'confidence': bot.last_ml_result.get('confidence', 0.0),
                    'probabilities': bot.last_ml_result.get('probabilities', []),
                    'current_strategy': bot.strategy_name,
                    'bot_name': bot.name,
                }
            else:
                # Bot exists but no ML result yet
                result[str(bot_id)] = {
                    'state': -1,
                    'confidence': 0.0,
                    'probabilities': [],
                    'current_strategy': bot.strategy_name if hasattr(bot, 'strategy_name') else 'Unknown',
                    'bot_name': bot.name if hasattr(bot, 'name') else f'Bot {bot_id}',
                    'waiting': True
                }
        return jsonify({
            'success': True,
